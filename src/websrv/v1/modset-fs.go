...
func (c ModSetFs) RenameAction() {
	...
	path = filepath.Clean(conf.Config.ModuleDir + "/" + modname + "/" + path)

	pathset := filepath.Clean(req.PathSet)
	pathset = filepath.Clean(conf.Config.ModuleDir + "/" + modname + "/" + pathset)

	dir := filepath.Dir(pathset)
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		fsMakeDir(dir, conf.User.Uid, conf.User.Gid, 0750)
	}
...
func fsFilePutWrite(path string, body []byte) error {
	...
	iUid, _ := strconv.Atoi(conf.User.Uid)
	iGid, _ := strconv.Atoi(conf.User.Gid)
...
func (c ModSetFs) ListAction() {
	...
	projfp := filepath.Clean(conf.Config.ModuleDir + "/" + modname + "/" + path)
