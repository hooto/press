...
func (c Comment) EmbedAction() {
	...
	if c.Params.Get("refer_specid") == "" || c.Params.Get("refer_id") == "" {
		return
	}

	qry := datax.NewQuery(nsSpecID, "entry")
	qry.Limit(500)
	qry.Order("created asc")
	qry.Filter("field_refer_id", c.Params.Get("refer_id"))
	qry.Filter("field_refer", c.Params.Get("refer_specid")+"."+c.Params.Get("refer_datax_table"))

	c.Data["list"] = qry.NodeList()

	c.Data["new_form_refer_id"] = c.Params.Get("refer_id")
	c.Data["new_form_refer_specid"] = c.Params.Get("refer_specid")
...
func (c Comment) SetAction() {
	...
	if _, ok := conf.Instances[set.ReferSpecID]; !ok {
		set.Error = &types.ErrorMeta{
			Code:    "400",
	...
	re_title := "Re: "
	prevq := datax.NewQuery(set.ReferSpecID, set.ReferDataxTable)
	...
	item := map[string]interface{}{
		...
		"field_refer":    set.ReferSpecID + "." + set.ReferDataxTable,
	...
	if _, err := dcn.Base.Insert("nxcomment_entry", item); err != nil {
		set.Error = &types.ErrorMeta{
			Code:    "500",
			Message: err.Error(),
		}
