...
<div id="hpm-termset" style="box-sizing: border-box;">loading</div>

<div style="margin:20px 0;">
  <button class="pure-button btapm-btn btapm-btn-primary" onclick="hpTerm.SetCommit()">Save</button>
  <button class="pure-button btapm-btn" onclick="hpTerm.List()">Cancel</button>
</div>
    ...
    <p>
      <select name="pid" class="l4i-form-control">
        ...
        <option value="{[=v.id]}" {[ if (it.pid == v.id) { ]}selected{[ } ]}>{[=v.title]}</option>
          ...
          {[~v._subs :v2]}
          {[ if (v2.id != it.id) { ]}
          <option value="{[=v2.id]}" {[ if (it.pid == v2.id) { ]}selected{[ } ]}>{[=hpTerm.Sprint(v2._dp)]}{[=v2.title]}</option>
          {[ } ]}
          {[~]}
          {[?]}
        {[ } ]}
