...
var hpressSpecEditor = {

}

hpressSpecEditor.Open = function(modname) {
    var topnav = $("#hpress-uh-topnav");
    ...
    if (topnav.find("a[modname='" + modname + "']").length > 0) {
        ...
        hpressSpecEditor.Index(modname);
        return;
    }
    ...
    $("#hpress-uh-topnav").append("<a class=\"l4i-nav-item active\" modname=\"" + modname + "\" href=\"#spec-editor/" + modname + "\">Spec Editor (" + modname + ")</a>");
...
hpressSpecEditor.Index = function(modname) {
    if (!modname) {
        return;
    }
    ...
    l4iSession.Set("hpress-speceditor-modname", modname);
    l4iSession.Set("modname_current", "/");

    hpressMgr.TplCmd("spec/editor/desk", {
        callback: function(err, data) {
            ...
            l4iTemplate.RenderFromID("lcbind-proj-filenav", "hpress-speceditor-fsnav-tpl");
...
l9rProjFs.OpenHistoryTabs = function() {
    ...
    lcData.Query("files", "projdir", l4iSession.Get("hpress-speceditor-modname"), function(ret) {
        ...
        if (ret.value.id && ret.value.projdir == l4iSession.Get("hpress-speceditor-modname")) {
            ...
            var tabLastActive = l4iStorage.Get(l4iSession.Get("hpress-speceditor-modname") + ".cab." + ret.value.cabid);
...
l9rPodFs.Get = function(options) {
    ...
    var url = "mod-set-fs/get?modname=" + l4iSession.Get("hpress-speceditor-modname");
    // url += "?access_token="+ l4iCookie.Get("access_token");
    url += "&path=" + options.path;

    // console.log("box refresh:"+ url);
    hpressMgr.ApiCmd(url, {
        callback: function(err, data) {
...
l9rPodFs.Post = function(options) {
    ...
    var url = "mod-set-fs/put?modname=" + l4iSession.Get("hpress-speceditor-modname");
...
l9rPodFs.Rename = function(options) {
    ...
    var url = "mod-set-fs/rename?modname=" + l4iSession.Get("hpress-speceditor-modname");
    hpressMgr.ApiCmd(url, {
        method: "POST",
        timeout: 10000,
        data: JSON.stringify(req),
...
l9rPodFs.Del = function(options) {
    ...
    var url = "mod-set-fs/del?modname=" + l4iSession.Get("hpress-speceditor-modname");
...
l9rPodFs.List = function(options) {
    ...
    var url = "mod-set-fs/list?modname=" + l4iSession.Get("hpress-speceditor-modname");
    url += "&path=" + options.path;
...
l9rLayout.Initialize = function() {
    ...
    for (var i in l9rLayout.cols) {
        ...
        var wl = l4iStorage.Get(l4iSession.Get("hpress-speceditor-modname") + "_laysize_" + l9rLayout.cols[i].id);
...
l9rLayout.BindRefresh = function() {
    $(".lclay-col-resize").bind("mousedown", function(e) {
        ...
        $("#lcbind-layout").bind("mousemove", function(e) {
            ...
            l4iStorage.Set(l4iSession.Get("hpress-speceditor-modname") + "_laysize_" + leftLayId, leftWidthNew);
            l4iSession.Set("laysize_" + leftLayId, leftWidthNew);
            l4iStorage.Set(l4iSession.Get("hpress-speceditor-modname") + "_laysize_" + rightLayId, rightWidthNew);
