...
l9rTab.Open = function(options) {
    ...
    var urid = l4iString.CryptoMd5(l4iSession.Get("hpress-speceditor-modname") + options.uri);
    ...
    if (!l9rTab.pool[urid]) {
        ...
        l9rTab.pool[urid] = {
            modname: l4iSession.Get("hpress-speceditor-modname"),
            url: options.uri,
            colid: options.colid,
...
l9rTab.Switch = function(urid) {
    ...
    if (item.modname != l4iSession.Get("hpress-speceditor-modname")) {
        return;
    }
    ...
    switch (item.type) {
        ...
        case "editor":
            ...
            lcEditor.TabletOpen(urid, function(ret) {
                ...
                l4iStorage.Set(l4iSession.Get("hpress-speceditor-modname") + ".cab." + item.target, urid);
...
l9rTab.TabletTitleImage = function(urid, imgsrc) {
    ...
    if (imgsrc === undefined && item.icon !== undefined) {
        ...
        if (item.icon.slice(0, 1) == "/") {
            ...
            imgsrc = hpressMgr.frtbase + "~/hpressm/img/" + item.icon + ".png";
        }
    }
...
l9rTab.TabletTitle = function(urid, loading) {
    ...
    if ($("#pgtab" + urid).length < 1) {
        ...
        if (item.icon) {
            ...
            if (loading) {
                var imgsrc = hpressMgr.frtbase + "~/hpressm/img/loading4.gif";
            } else {
                var imgsrc = hpressMgr.frtbase + "~/hpressm/img/" + item.icon + ".png";
            }
...
l9rTab.TabletMore = function(tg) {
    ...
    for (i in l9rTab.pool) {
        ...
        ol += '<div class="ltm-ico"><img src="' + hpressMgr.frtbase + '~/hpressm/img/' + l9rTab.pool[i].icon + '.png" align="absmiddle" /></div>';
            ...
            if (force == 1) {
                ...
                lcEditor.IsSaved(urid, function(ret) {
                    ...
                    l4iModal.Open({
                        ...
                        tpluri: hpressMgr.frtbase + "~/hpressm/editor/changes2save.tpl",
                        width: 500,
                        height: 180,
                        data: {
                            urid: urid
